# Segmentation DataLoader - COCO JSON format
# Source: github.com/Clarifai/examples/datasets/upload/image_segmentation/coco/dataset.py
#
...
class COCOSegmentationDataLoader(ClarifaiDataLoader):
    ...
    def __init__(self, split: str = "train"):
        ...
        self.annotations_file = {
            "train": os.path.join(os.path.dirname(__file__), "annotations/instances_val2017_subset.json")
        }
        self.load_data()
            ...
            if len(annot_ids) > 0:
                ...
                for ann in img_annotations:
                    ...
                    if type(ann['segmentation']) == list:
                        ...
                        annots.append(polygons.tolist())
                        class_names.append(self.cat_id_map[cat_id])
            else:
                continue

        assert len(class_names) == len(annots), \
            f"Num classes must match num annotations. Found {len(class_names)} classes and {len(annots)} polygons."
