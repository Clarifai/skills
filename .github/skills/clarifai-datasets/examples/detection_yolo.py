# Detection DataLoader - YOLO TXT format (MultiFolderDetection style)
# Source: github.com/Clarifai/examples/datasets/upload/image_detection/multifolder/dataset.py
#
# Data structure:
#   multifolder/
...
#           └── image2.txt
#
# YOLO TXT format (one line per object):
...
class MultiFolderDetectionDataLoader(ClarifaiDataLoader):
    """Multifolder Images Detection Dataset - YOLO TXT format."""

    def __init__(self, root_dir):
        """
        Args:
            root_dir: Directory containing multiple subdirectories,
                     each with images and corresponding annotation files.
        """
    ...
    def __getitem__(self, index: int):
        ...
        with open(annot_path, 'r') as f:
            for line in f.readlines():
                ...
                # to Clarifai format (left_col, top_row, right_col, bottom_row)
        ...
        assert len(concept_ids) == len(annots), \
            f"Num concepts must match num bbox annotations. Found {len(concept_ids)} concepts and {len(annots)} bboxes."
